...
$app->match('/orders', function() use ($app) {
	...
	if ($app['session']->get('is_user')) {
		...
		// Look up customer's pending orders
        $db = $app['db'];
		$query = "select toy.toynum as tnum, name, date_ordered
					from toy, orders
					where date_deliv = '0000-00-00' and
					toy.toynum=orders.toynum and
					custnum = ?";
		$pending = queryDB($db, $query, array($cnum));
		
		// Look up customer's delivered orders
		$query = "select toy.toynum as tnum, name, date_deliv
					from toy, orders
					where date_deliv != '0000-00-00' and
					toy.toynum=orders.toynum and
					custnum = ?";
		$delivered = queryDB($db, $query, array($cnum));

	}
	
	return $app['twig']->render('orders.html.twig', array(
		'pageTitle' => 'Orders',
		'pending' => $pending,
		'delivered' => $delivered
	));
